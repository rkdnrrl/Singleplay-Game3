...
const NAME_CATCH_OBJ_GROUPS = {
  ...
  전자: `모니터 TV 프로젝터 스피커 사운드바 헤드폰 이어폰 이어버드 마이크 웹캠 마우스 패드 게이밍마우스 무선마우스 키보드 노트북 태블릿 스마트폰 스마트워치 충전기 어댑터 보조배터리 파워뱅크 멀티탭 연장코드 콘센트 USB허브 랜허브 공유기 외장하드 SSD 메모리카드 USB메모리 카드리더기 프린터 스캐너 복합기 팩스 리모컨 셋톱박스 블루레이플레이어 DVD플레이어 CD플레이어 라디오 시계 라디오시계 액션캠 블루투스스피커`,
  ...
  고철: `와이어코일 케이블릴 베어링 롤러베어링 볼베어링 톱니바퀴 기어 스프로킷 체인 체인링크 I빔 H빔 앵글 철근 형강 압연재 압연권 슬래그덩이 플랜지 플랜지판 게이트밸브 볼밸브 밸브캡 압력게이지 샤프트 키웨이 커플링 브라켓 L브라켓 용접봉 전극 퓨즈박스 단자대 녹철판 철판절단편 알루미늄각재 동파이프 강관 와이어로프 스프링와셔 볼트너트세트 리벳 드릴비트 소켓 렌치헤드 톱날 체인톱날`,
};
