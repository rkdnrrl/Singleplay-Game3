(function () {
  ...
  /** 화학 토큰만 이어붙여 분해 시 기호가 여러 개 나오게 함 (산소통·물통·이산화탄소수소통 등) */
  const FORMULA_CHUNK = [
    '이산화탄소', '이산화질소', '이산화황', '일산화탄소', '일산화질소',
    '산소', '수소', '질소', '탄소', '물', '암모니아', '메탄', '아세틸렌', '황산',
    '철', '구리', '아연', '알루미늄', '주석', '납', '니켈', '크롬', '망간', '은', '금', '티타늄', '코발트', '동',
    '나트륨', '칼륨', '칼슘', '마그네슘', '리튬', '염소', '불소', '인', '황', '브로민', '요오드',
  ];
  /** 기호 없이 붙는 말미 — 산소통·물병 느낌 */
  const FORMULA_VESSEL_SUFFIX = [
    '통', '병', '탱크', '관', '캔', '드럼', '포대', '용기', '호스', '함', '줄', '박스',
  ];
  ...
  const FORMULA_SHAPE = [
    '검', '방패', '판', '막대', '가루', '덩어리', '코일', '링', '볼트', '너트', '관', '선', '주괴', '슬래그', '리바', '칼', '창', '사슬',
  ];
  const FORMULA_SINGLE = [
    '고철', '비철', '청동', '황동', '백동', '강철', '산화철', '스테인리스',
  ];
  ...
  function pickDistinctMany(pool, used) {
    const set = new Set((Array.isArray(used) ? used : [used]).map(String));
    const filt = pool.filter((x) => !set.has(x));
    return pick(filt.length ? filt : pool);
  }
  ...
  /**
   * 줍는 고철 이름 — 화학 토큰만 이어 붙여 `catchNameToFormula`에 여러 기호가 나오게 함.
   * (예: 산소통·물통·이산화탄소수소통·구리아연병)
   */
  function generateCatchName() {
    const builders = [
      () => {
        const a = pick(FORMULA_CHUNK);
        const b = pickDistinctMany(FORMULA_CHUNK, [a]);
        const c = pickDistinctMany(FORMULA_CHUNK, [a, b]);
        return `${a}${b}${c}`;
      },
      () => {
        const a = pick(FORMULA_CHUNK);
        const b = pickDistinctMany(FORMULA_CHUNK, [a]);
        return `${a}${b}${pick(FORMULA_VESSEL_SUFFIX)}`;
      },
      () => `${pick(FORMULA_CHUNK)}${pick(FORMULA_VESSEL_SUFFIX)}`,
      () => `${pick(FORMULA_CHUNK)} ${pick(FORMULA_CHUNK)}${pick(FORMULA_VESSEL_SUFFIX)}`,
      () => `${pick(FORMULA_METAL)}${pick(FORMULA_SHAPE)}`,
      () => `${pick(FORMULA_METAL)} ${pick(FORMULA_SHAPE)}`,
      () => pick(FORMULA_SINGLE),
      () => {
        const a = pick(FORMULA_METAL);
        const b = pickDistinct(FORMULA_METAL, a);
        return `${a}${b}`;
      },
    ];
    ...
    for (let guard = 0; guard < 32; guard += 1) {
      const r = Math.random();
      let fn;
      if (r < 0.22) fn = builders[0];
      else if (r < 0.48) fn = builders[1];
      else if (r < 0.66) fn = builders[2];
      else if (r < 0.74) fn = builders[3];
      else if (r < 0.82) fn = builders[4];
      else if (r < 0.90) fn = builders[5];
      else if (r < 0.96) fn = builders[6];
      else fn = builders[7];
      name = fn();
      if (name && name.length <= 26) break;
    }
    if (!name) name = `${pick(FORMULA_CHUNK)}${pick(FORMULA_VESSEL_SUFFIX)}`;
    return name;
  }
  ...
  function generateBackgroundScrapName() {
    ...
    return name || `${pick(FORMULA_CHUNK)}${pick(FORMULA_VESSEL_SUFFIX)}`;
  }
